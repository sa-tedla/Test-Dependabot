...
export const createQAndAThreadSchema = z.object({
  contents: z
    .string()
    .trim()
...
export type CreateQAndAThreadSchema = z.infer<typeof createQAndAThreadSchema>;

export const updateQAndAAssistantSchema = z.object({
  assistantId: z.string(),
  indexId: z.string(),
  name: z
    .string()
  ...
  systemPrompt: z
    ...
    .max(1000, 'プロンプトは1000文字以内で入力してください')
    .refine(
      (content) => content.includes('{context}'),
      '文字列「{context}」をプロンプトに含めてください。'
    ),
  sources: z
    .array(
      z
...
export type UpdateQAndAAssistantSchema = z.infer<typeof updateQAndAAssistantSchema>;
